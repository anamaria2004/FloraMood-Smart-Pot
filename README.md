...
The LCD display has the pins named VCC connected to VCC, GND to GND, DIN to GP8, CLK to GP10, CS to GP9, DC to GP7, RST to GP12, BL to GP11. The light sensor has the AOUT connected to GP28, the temperature AOUT is connected to GP27 and the soil sensor's AOUT is on GP26. The relay module has the control pin GP0. 
For the software part, I started with importing the necessary libraries. Then, I defined the interrupts for USB and ADC. I defined a task which contains the logger, the conversion from adc to voltage, the soil moisture percentage obtained from voltage, and the function that computes the light. 
In the function main I declared the peripherals, the driver, then defined the adc. I declared the pins for the sensors and relay, and also configured the display. I set its style, orientation, background color, text color, wrote the messages to display. For displaying the frames to make a gif, I created a vector with the necessary raw images.
In the loop I go through the vector and display each image in order to have 60 frames per second. Also, I red the adc values for sensors, and converted them to the actual value. I set the intervals for printing each specific message based on the found values. 
Finally, I have the panic handler with the panic function. 
